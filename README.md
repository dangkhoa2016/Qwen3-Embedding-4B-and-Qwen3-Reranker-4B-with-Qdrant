...
**Qwen3-Embedding-4B and Qwen3-Reranker-4B with Qdrant** is a production-oriented retrieval stack built around Qwen3-Embedding-4B, Qwen3-Reranker-4B, and Qdrant. It provides a CPU-oriented FastAPI service plus a reproducible 20K-point Qdrant production-demo path qualified on a fresh Kaggle CPU session. It is an embedding/retrieval/reranking project, not a chat-LLM server.
